...
trainDF$isSpam <- revalue(trainDF$isSpam, c("T"="Spam", "F"="Ham"))
trainDF$isSpam <- relevel(trainDF$isSpam, "Spam")
testDF$isSpam <- revalue(testDF$isSpam, c("T"="Spam", "F"="Ham"))
testDF$isSpam <- relevel(testDF$isSpam, "Spam")
trainDF$perCaps <- log(1+trainDF$perCaps)
trainDF$bodyCharCt <- log(1+trainDF$bodyCharCt)
testDF$perCaps <- log(1+testDF$perCaps)
testDF$bodyCharCt <- log(1+testDF$bodyCharCt)
...
performance(spam.pred, measures = list(acc, auc, fpr, fnr))
# get scores
...
# calculateConfusionMatrix(spam.pred, relative = TRUE, sums = TRUE)

# viz
plotLearnerPrediction(spam.lrn, 
                      task = email.clf, 
...
thresh_plot = generateThreshVsPerfData(spam.pred, measures = list(fpr, fnr, mmce))
...
spam.base.clf = resample(learner = spam.lrn, 
                         task = spam.tsk, 
                         resampling = spam.resamp.bin, 
                         measures = list(mmce, fpr, fnr), 
                         models = TRUE) # keep models in getter = TRUE
                         #extract = FALSE)  # extract only certain data


names(spam.base.clf)
# overall performance:
spam.base.clf$aggr
performance(spam.base.clf$pred)

# perf for each fold
spam.base.clf$measures.test

thresh_plot = generateThreshVsPerfData(spam.base.clf$pred, 
                                       measures = list(fpr, fnr, mmce))
plotThreshVsPerf(thresh_plot)
...
# TUNING 
# identify search grid
# test individual params (if range makeNumericParam(param, lower=, upper=))
spam.ps = makeParamSet(
                  makeDiscreteParam("cp", values = c(0.00001, 0.0001, 0.001, 0.01, 0.1, 0.2)),
                  makeDiscreteParam("minsplit", values = c(1, 3, 5, 10, 15)),
                  makeDiscreteParam("maxdepth", values = c(2, 5, 7, 10, 15))
                 )
print(spam.ps)
...
plotTuneMultiCritResult(spam.tuned.clf)
getDefaultMeasure(email.clf)
listMeasures(email.clf)
