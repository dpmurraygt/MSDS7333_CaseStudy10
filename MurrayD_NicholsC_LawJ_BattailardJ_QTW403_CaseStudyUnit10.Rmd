...
emailDF = setupRpart(emailDF)

set.seed(418910) # set seed for reproducibility

spam = emailDF$isSpam == "T"
numSpam = sum(spam)
numHam = sum(!spam)
...
testDF = 
  rbind( emailDF[emailDF$isSpam == "T", ][testSpamIdx, ],
         emailDF[emailDF$isSpam == "F", ][testHamIdx, ] )
...
# preProcess(method="bagImpute", trainDF)
...
rpFit <- train(isSpam ~ ., data=trainDF, method="rpart", trControl = fitControl, na.action = na.omit)
