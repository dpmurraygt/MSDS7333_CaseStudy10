...
Utilizing the full listing of variables in our dataset can lead to overfitting, however, decision trees allow us to find the best variables for splitting while pruning extraneous splits. Applying the first three parameters previously mentioned typically reduces the size of the final tree. This reduction in variance should help with model generalization to the test dataset. 
...
For comparison purposes, an optimized model is also fit on the training data set. Optimization is achieved by exploring a discrete list of the four parameters of interest. A grid search method is used with ten-fold stratified cross-validation. The mean AUC for each model and associated combination of parameters is compared and the model with the highest cross-validated AUC is chosen.

table list of discrete vars here

The base model is then subsequently compared to the optimized model given the test data set.



needed?
Rcp(T) ≡ R(T) + cp ∗ |T| ∗ R(T1)
where T1 is the tree with no splits, |T| is the number of splits for a tree, and R is the risk. This scaled version is much more user friendly than the original CART formula (4.1) since it is unit less. A value of cp = 1 will always result in a tree with no splits.
