---
title: "Exploring rpart and CART for Spam Detection"
...
```{r load_libs, include=FALSE, cache=TRUE, warning=FALSE, echo=FALSE}
library(magrittr)
library(reshape2)
library(plyr)
library(dplyr)
...
Rather than discard over three percent of the dataset, we undertake imputation using random forest methods for both numeric and categorical predictor variables.
...
# impute, this has been cached but runs each time so we saved output
...
```{r chi_sq_cats, include=TRUE, cache=TRUE, echo=FALSE, fig.height=8, fig.width=8}
...
Lower p-values indicate we reject the null hypothesis of random association. Obviously, significant non-random dependence between factor variables exists. This makes logical sense for variables such as isWrote, which indicates if an email is electronically scribed. Since the majority of emails are electronically scribed, we can assume this variable is not as important for predicting spam or ham. Other interesting relationships such as the independence between priority and noHost indicate that these variables may be useful separately for analysis. 
...
```{r biser_cor, include=TRUE, echo=FALSE, cache=TRUE, fig.height=8, fig.width=8.5}
...
In summary, highly correlated predictor variables could be extraneous to our objective of predicting spam. To address predictor and response relationships, we visually inspect both continuous and factor variable relationships with isSpam in the next section. Additionally, we establish variable importances using the rpart package.
...
```{r fit_dtree_base, echo=FALSE, include=TRUE, fig.width=8.5, fig.height=5}
set.seed(4)
...
Our base model lists perCaps as the most important variable to split in Figure 7. Behind it is BodyCharCt, which are the number of characters in the body of the email message. Given these variables are so important, we visually inspect a default rpart model fit on our training data set using perCaps and BodyCharCt.

```{r perCaps_class, include=TRUE, echo=FALSE}
...
g+
  ggtitle('Figure 8: Body Character Count and Percent Capitals')
```

We represent values on a log scale in order to provide a clean visualization of classification areas. The observations outlined in white are misclassifications and the color boundaries represent isSpam classes of spam and not spam (ham). The lighter areas of the chart represent lower probabilities for a given class. Obviously the middle of the chart represents an area of major misclassification. Additionally, utilizing only these two variables, our base model struggles with false positive rate. This indicates our model needs more complexity. However, these two variables alone provide for good separation as part of a more complex model.

Thus, we leave it up to rpart and its default parameters to create a decision tree containing 14 splits represented in Figure 9. This default fit is considered our base model.

```{r tree_plot, include=TRUE, echo=FALSE, fig.width=8.5, fig.height=5}
prp(splits, 0, extra=1)
```

Of particular note is the base model's usage of bodyCharCt. It is used three times in the splitting process. Additionally, we can see quite a bit of misclassification in the model on the training set, particularly a prevalence of false negatives. We utilize the test set to investigate a confusion matrix and model performance in Figure X to address this issue.
...
As stated previously, we explore the complexity parameter, minimum split for each node, maximum depth of the tree and the splitting criterion via a grid search. The parameter list is given in Table XX below:
...
Optimization begins to stall just under an AUC of 0.97 for our training data. Our optimized tree is much larger than our base model. In fact, 76 total splits are used for our optimized tree. This is because the optimal complexity penalty is set low at 0.001. This may be a risk for overfitting our test set, which we will explore shortly.

```{r parms_look, echo=FALSE, include = TRUE}

splits <- getLearnerModel(spam.opt.clf)

df_models[df_models$auc.test.mean > 0.96,] %>% 
      ggplot(aes(x=minsplit, y=auc.test.mean)) + 
      geom_col() + 
      #facet_wrap(~Predictor, scales = "free_y", ncol=5) + 
      theme_light() + 
      ggtitle("Figure 6: Continous Predictor Variables and Spam Outcomes") + 
...
Of the 29 models fit during the tuning process with a resulting cross-validated AUC of greater than 0.96, all have complexity parameters of 0.001. Additionally the maxdepth for a decision tree node is always 15 or higher. 14 out of the 29 models use the information splitting criterion and the majority of models have a minsplit value of 15 or higher. This result indicates the complexity parameter tends to dominate when it comes to maximizing AUC results for our analysis task. Optimization favors more complex models with deeper nodes. However, the splitting criterion is nearly a toss up.

We can easily see the complexity parameter's dominance in Figure XX below, where the mean AUC across validation splits is maximized as long as the cp remains low. 

```{r cp_vs_minsplit, echo=FALSE, include=TRUE}
...
After thoroughly exploring rpart parameters, we turn to generalization performance. Given an AUC of 0.95 for our base model, we fit our optimized model to the same training data set of emails. Results 
...
While our objective was accomplished, some cautious advice must be given. 
...
```{r dtree_explore, include=FALSE, eval=FALSE}
...
```{r nbayes_get_data, include=FALSE, eval=FALSE}
...
``` {r nbayes_train_test_split, include=FALSE, eval=FALSE}
set.seed(418910)
...
```{r nbayes_freq_table, include=FALSE, eval=FALSE}
...
```{r nbayes_classify, include=FALSE, eval=FALSE}
